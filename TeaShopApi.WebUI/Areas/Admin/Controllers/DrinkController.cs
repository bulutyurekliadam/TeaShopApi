...
namespace TeaShopApi.WebUI.Areas.Admin.Controllers
{
	...
	public class DrinkController : Controller
	{
        ...
        public async Task<IActionResult> Index()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:7001/api/Drinks");
		...
		public async Task<IActionResult> CreateDrink(CreateDrinkDto createDrinkDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7001/api/Drinks", content);
			if(responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync("https://localhost:7001/api/Drinks?id="+id);
			if(responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
            }
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync("https://localhost:7001/api/Drinks/" + id);
			if(responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateDrink(UpdateDrinkDto updateDrinkDto)
        {
			...
			var responseMessage = await client.PutAsync("https://localhost:7001/api/Drinks/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
				return RedirectToAction("Index");
            }
